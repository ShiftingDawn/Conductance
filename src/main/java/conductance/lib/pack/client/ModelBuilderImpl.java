...
final class ModelBuilderImpl extends JsonResourceBuilderImpl<ModelBuilder> implements ModelBuilder {
	...
	private @Nullable ResourceLocation loader;
	private @Nullable CompositeModelBuilderImpl compositeBuilder;
	private @Nullable ResourceLocation renderType;
	private @Nullable Boolean ambientOcclusion;
	private @Nullable BlockModel.GuiLight guiLight;
	...
	@Override
	public ModelBuilder texture(final String referenceKey, final String textureOrReferenceKey) {
		this.textures.put(referenceKey, textureOrReferenceKey);
		return this;
	}
	...
	protected void populateJson(final JsonObject json) {
		...
		if (!this.displays.isEmpty()) {
			json.add("display", Util.make(new JsonObject(), displayJson -> {
				this.displays.forEach((key, builder) -> displayJson.add(key.getSerializedName(), builder.build()));
			}));
		}
		...
		if (!this.elements.isEmpty()) {
			json.add("elements", Util.make(new JsonArray(), array -> this.elements.forEach(element -> array.add(element.build()))));
		}
