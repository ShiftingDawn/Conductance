...
@Getter
final class RecipeElementTypeSerializer<T> extends RegistryObject<ResourceLocation> implements IRecipeElementType<T> {

	private final Codec<T> dataCodec;
	private final StreamCodec<RegistryFriendlyByteBuf, T> dataStreamCodec;
	private final RecipeElementCloner<T> cloner;

	RecipeElementTypeSerializer(final ResourceLocation registryKey, final Codec<T> codec, final StreamCodec<RegistryFriendlyByteBuf, T> dataStreamCodec, final RecipeElementCloner<T> cloner) {
		super(registryKey);
