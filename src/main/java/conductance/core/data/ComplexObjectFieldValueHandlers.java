...
final class ComplexObjectFieldValueHandlers {

	public static class HandlerEnum implements ManagedFieldValueHandler<Enum<?>> {
		...
		@Override
		public Tag serialize(final Enum<?> value) {
			return StringTag.valueOf(value.toString().toLowerCase(Locale.ROOT));
		}

		@Override
		public @Nullable Enum<?> deserialize(final Tag nbt) {
			return null;
		}

		@Override
		public @Nullable Enum<?> deserialize(final Class<Enum<?>> fieldType, final Tag nbt) {
	...
	private ComplexObjectFieldValueHandlers() {
	}
}
